...
class logisticRegression {
    ...
    predict(observations) {
        return this.processFeatures(observations)
            .matMul(this.weights)
            .sigmoid()
            .greater(this.options.decisionBoundary) //decision boundary - 0.5 above to 1 and below 0.
            .cast("float32"); // from bool to float Numbers
    }
    ...
    recordCrossentropy() {
        ...
        const termOne = this.labels.transpose().matMul(guesses.log());  // Actual mul log(guess)

        const termTwo = this.labels
            .mul(-1)
            .add(1)
            .transpose()
            .matMul(guesses.mul(-1).add(1).log());                    //(1- Actual) mul log(1 - guess)
